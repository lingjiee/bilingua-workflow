...
- 审校补丁原子验证：所有修订块先通过与源文、冻结术语绑定的门禁，再统一写入日志。
- 跨块一致性门禁：重复英文块的多译会阻断构建；高频术语分歧进入人工审核报告。
...
每次构建都会生成与源文件哈希、冻结术语版本绑定的 `review-context.json`。
`apply-review` 会先合并整份补丁，在内存中验证所有修订块；任一错误都会拒绝整份补丁，
不会留下只导入一部分的状态。跨块候选会写入 `review-verify-report.md`。
...
`0.3.1` 为首个公开发布版本。核心流水线已经在多本长篇图书上完成实际验证，但仍属于 alpha：建议所有新书先跑代表章节验收，并保留独立人工抽查。
