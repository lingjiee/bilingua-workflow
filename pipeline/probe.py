...
def probe_shape(base: str, key: str, prefix: str, model: str) -> dict:
    ...
    for auth, mk in AUTH_STYLES.items():
        attempts.append((
            "anthropic", auth, f"{base}{prefix}/messages",
            {**mk(key), "anthropic-version": "2023-06-01",
    ...
    for auth, mk in AUTH_STYLES.items():
        attempts.append((
            "openai", auth, f"{base}{prefix}/chat/completions",
    ...
    for shape, auth, url, headers, body in attempts:
        ...
        ok = status == 200
        mark = "✓" if ok else "·"
        print(f"  {mark} {shape:9s} + {auth:9s} → {status or 'ERR'}"
              f"  {'' if ok else err_text(payload)[:110]}")
        if ok and not winner:
            winner = {"shape": shape, "auth": auth, "url": url,
                      "headers": headers, "sample": payload}
    if not winner:
        print("  ✗ 四种组合都没通")
    return winner
...
def probe_batch(base: str, key: str, prefix: str, auth: str, shape: str) -> bool:
    ...
    headers = {**AUTH_STYLES[auth](key), "content-type": "application/json",
...
def probe_caching(base: str, key: str, prefix: str, auth: str,
                  shape: str, model: str) -> bool:
    """D3 的成本论证一半靠 prompt caching。一致性论证不受影响。"""
    print("\n[4/4] Prompt caching（设计 D3 的成本论证依赖）")
    if shape != "anthropic":
        print("  · OpenAI 协议没有 cache_control 字段 → 成本论证减弱，一致性论证不变")
        return False
    headers = {**AUTH_STYLES[auth](key), "anthropic-version": "2023-06-01",
               "content-type": "application/json"}
    filler = ("这是一段用来撑过 prompt cache 最小长度门槛的填充文本，"
              "内容本身没有意义，只为凑够 token 数。") * 90
    body = {
        "model": model, "max_tokens": 16,
    ...
    for i in (1, 2):
        ...
        if status != 200:
            print(f"  ✗ 第 {i} 次带 cache_control 的请求 → {status}"
                  f"  {err_text(payload)[:110]}")
