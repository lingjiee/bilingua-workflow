...
本仓库以 Apache-2.0 公开发布。公开前已完成版权审计、许可证选择与全历史密钥扫描；
跟踪内容仅限通用代码、测试、文档与合成示例。

从本仓库派生私有翻译项目时，首次同步仍应默认使用 private 仓库：公开与否不改变版权边界，
私有也不等于可以上传未经授权的原书。改为 public 前须重新完成版权审计与历史扫描。
